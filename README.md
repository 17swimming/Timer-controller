...
6. 可自定义提醒时间间隔的活动记录提醒功能
   ...
   - 通过醒目的弹窗形式进行提醒
   - 提醒弹窗具有动画效果，确保用户注意到
   - 下一次提醒时间基于用户最后一次记录活动的时间点计算，而非机械地按固定间隔提醒
...
8. 查看时间轴图表，可以看到每个小时内的活动分布情况
   - 时间线视图采用类似GitHub贡献图的风格，每个小时划分为4个15分钟的小方块
   - 每个小方块代表15分钟内主要进行的活动，根据活动类别进行着色
   - 纵坐标表示时间占用比例，从0分钟到60分钟
   - 横坐标表示时间线，从开始记录时间到结束记录时间
9. 悬浮球功能增强：
   - 悬浮球默认尺寸为100px，显示应用图标，更加醒目
   - 支持通过鼠标拖拽移动悬浮球到屏幕任意位置
...
所有数据都存储在用户数据目录中的 db.json 文件中，包含：
- 活动记录
- 待办事项
- 已完成任务
- 应用设置

在Windows系统中，用户数据目录通常位于：
- Windows 7/8/10/11: `C:\Users\<用户名>\AppData\Roaming\timer-controller\`

当应用更新时，用户数据目录中的数据会保留，不会丢失。

## 便携性

构建后的应用为便携式，可直接运行无需安装，所有数据都存储在用户数据目录中。